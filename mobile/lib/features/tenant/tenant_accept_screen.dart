...
class _TenantAcceptScreenState extends ConsumerState<TenantAcceptScreen> {
  ...
  Future<void> _accept() async {
    ...
    if (invite == null) {
      showErrorSnackBar(context, 'That code doesn\'t look right — check it was copied in full.');
      return;
    }
    ...
    try {
      ...
      if (mounted) context.go('/tenant/${invite.organizationId}/home');
    } catch (e) {
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            const Text('Paste the invite code your PG owner shared with you.'),
            const SizedBox(height: 16),
            TextField(
              controller: _code,
              maxLines: 4,
              decoration: const InputDecoration(border: OutlineInputBorder(), hintText: 'Paste invite code here'),
            ),

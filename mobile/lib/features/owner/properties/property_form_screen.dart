...
class _PropertyFormScreenState extends ConsumerState<PropertyFormScreen> {
  ...
  late final _line1 = TextEditingController(text: widget.existing?.address.line1);
  late final _line2 = TextEditingController(text: widget.existing?.address.line2);
  late final _city = TextEditingController(text: widget.existing?.address.city);
  late final _state = TextEditingController(text: widget.existing?.address.state);
  late final _pincode = TextEditingController(text: widget.existing?.address.pincode);
  late final _phone = TextEditingController(text: widget.existing?.phone);
  late final _rentDueDay = TextEditingController(text: (widget.existing?.rentDueDay ?? 5).toString());
  late final _gracePeriodDays = TextEditingController(text: (widget.existing?.gracePeriodDays ?? 3).toString());
  ...
  @override
  void dispose() {
    for (final c in [_name, _slug, _line1, _line2, _city, _state, _pincode, _phone, _rentDueDay, _gracePeriodDays]) {
      c.dispose();
    }
    super.dispose();
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (_isEdit) {
        await api.dio.patch('/organizations/${widget.orgId}/properties/${widget.existing!.id}', data: data);
      } else {
        await api.dio.post('/organizations/${widget.orgId}/properties', data: data);
      }
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            TextFormField(
              controller: _slug,
              decoration: const InputDecoration(labelText: 'Slug (lowercase, hyphens only)'),
              onChanged: (_) => _slugEdited = true,
              validator: (v) {
                if (v == null || v.trim().isEmpty) return 'Required';
                if (!RegExp(r'^[a-z0-9-]+$').hasMatch(v.trim())) return 'Lowercase letters, numbers, hyphens only';
                return null;
              },
            ),
            const SizedBox(height: 12),
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(labelText: 'Rent due day of month (1-28)'),
              keyboardType: TextInputType.number,
              validator: (v) {
